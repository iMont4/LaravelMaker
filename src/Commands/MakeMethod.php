...
class MakeMethod extends Command
{
    const PATHS = [
        ...
        'full_file_path' => [
            ...
            'request_index'  => 'app/Http/Requests/%route_kind%/%namespace%/%name%/%name%%method%Request.php',
    ...
    private function getDynamicKind()
    {
        $kinds = [
            'Get'  => [
                // according id
                false => ['request_index', 'resource_index'],
                true  => 'resource_show',
            ],
            'Post' => [
                // according id
    ...
    private function generateStub() :void
    {
        foreach (self::STUBS as $class) {
            if (in_array($class, ['dynamic', 'request'])) {
                if (!($classes = $this->getDynamicKind()))
                    continue;
    ...
    private function generateReplaces($class = NULL)
    {
        ...
        $replaces = [
            ...
            'DummyRequestIndexName'  => "{$this->model}{$this->method}Request",
